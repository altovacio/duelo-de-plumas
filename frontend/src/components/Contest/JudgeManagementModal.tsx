...
const JudgeManagementModal: React.FC<JudgeManagementModalProps> = ({
  ...
  const handleAddUserAsJudge = async (user: User) => {
    try {
              ...
              <div className="mb-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <UserSearch
                      onUserSelect={handleAddUserAsJudge}
                      placeholder="Type username or email..."
                      excludeUserIds={judges.filter(j => j.user_judge_id).map(j => j.user_judge_id!)}
                    />
                  </div>
                  
                  {/* Add Current User */}
                  {user && !judges.some(judge => judge.user_judge_id === user.id) && (
                    <div className="flex items-center justify-between p-3 bg-gray-50 rounded-md">
                      <span className="text-sm text-gray-600">Add yourself as a judge:</span>
                      <button
                        onClick={() => handleAddUserAsJudge(user)}
                        disabled={isAddingJudgeId !== null}
                        className="px-3 py-1 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50 text-sm"
                      >
                        {isAddingJudgeId === user.id ? 'Adding...' : 'Add Me'}
                      </button>
                    </div>
                  )}
                </div>
