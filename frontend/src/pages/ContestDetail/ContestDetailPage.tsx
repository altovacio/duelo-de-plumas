...
const ContestDetailPage: React.FC = () => {
    ...
    // Add state for auto-refresh functionality
    ...
    // Auto-refresh functionality for evaluation contests
    useEffect(() => {
      if (contest?.status === 'evaluation' && isAuthenticated) {
        // Set up polling to check if contest status changes
        const interval = setInterval(async () => {
          try {
            const updatedContest = await contestService.getContest(parseInt(id || '1'), password);
            if (updatedContest.status !== contest.status) {
              // Contest status changed, refresh the page
              console.log(`Contest status changed from ${contest.status} to ${updatedContest.status}, refreshing...`);
              fetchContestDetails(password);
            }
          } catch (error) {
            console.warn('Error checking contest status during auto-refresh:', error);
          }
        ...
        // Clear interval if contest is not in evaluation or user is not authenticated
        if (autoRefreshInterval) {
          clearInterval(autoRefreshInterval);
          setAutoRefreshInterval(null);
        }
      }
    }, [contest?.status, isAuthenticated, id, password]);
    ...
    return (
      <div>
        ...
        {/* Expired Contest Warning */}
        {contest.end_date && new Date() > new Date(contest.end_date) && contest.status === 'open' && (
          <div className="bg-red-50 border-l-4 border-red-400 p-4 mb-8">
            <div className="flex">
              ...
              <div className="ml-3">
                <h3 className="text-sm font-medium text-red-800">Contest Deadline Has Passed</h3>
                <div className="mt-2 text-sm text-red-700">
                  <p>
                    This contest's deadline was {new Date(contest.end_date).toLocaleDateString()} but it's still accepting submissions. 
                    {(user?.is_admin || user?.id === contest?.creator.id) && (
                      <span className="font-medium"> As the contest creator, you should consider moving this contest to evaluation phase.</span>
                    )}
                  </p>
                </div>
              </div>
                        ...
                        <button 
                          ...
                          className={`inline-block px-4 py-2 rounded-lg ${
                            contest.judge_restrictions && isJudge
                              ...
                              : contest.end_date && new Date() > new Date(contest.end_date)
                                ? 'bg-red-300 text-red-700 cursor-not-allowed border-2 border-red-400'
                                : 'bg-indigo-600 text-white hover:bg-indigo-700'
                          }`}
                          disabled={(contest.judge_restrictions && isJudge) || (contest.end_date !== null && new Date() > new Date(contest.end_date))}
                          title={
                            contest.judge_restrictions && isJudge 
                              ? 'Judges cannot submit texts to this contest due to judge restrictions'
                              : contest.end_date && new Date() > new Date(contest.end_date)
                                ? `Contest deadline has passed (${new Date(contest.end_date).toLocaleDateString()}). Submissions are no longer accepted.`
                                : contest.end_date 
                                  ? `Submit your text before the deadline: ${new Date(contest.end_date).toLocaleDateString()}`
                                  : 'Submit your text to this contest'
                          }
                        >
                          {contest.end_date && new Date() > new Date(contest.end_date) ? '⏰ Expired - Cannot Submit' : 'Submit Text'}
                        </button>
                      </div>
                    );
                  }
