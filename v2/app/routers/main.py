...
@router.get(
    "/roadmap/items", 
    response_model=List[roadmap_schemas.RoadmapItem], 
    tags=["Roadmap"]
)
async def get_roadmap_items(): # No dependency needed for reading
    """Retrieves all roadmap items."""
    # Directly call the synchronous service function
    # FastAPI handles running sync functions in a threadpool
    items = roadmap_service.get_roadmap_items()
    # Pydantic will validate the list of dicts against the response_model
    return items 
...
async def add_roadmap_item(
    ...
    new_item_dict = roadmap_service.add_roadmap_item(item_data.model_dump())
    if new_item_dict is None:
        # Handle potential error from service function (e.g., invalid input)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, 
            detail="Failed to add roadmap item. Invalid data?"
        )
    # Pydantic validates the returned dict against the response_model
...
async def delete_roadmap_item(
    ...
    """Deletes a roadmap item by ID."""
    deleted = roadmap_service.delete_roadmap_item(item_id)
    if not deleted:
        raise HTTPException(
...
@router.put(
    "/roadmap/item/{item_id}/status", 
    response_model=roadmap_schemas.RoadmapItem, # Return the updated item
    tags=["Roadmap"]
)
async def update_roadmap_item_status(
    ...
    updated = roadmap_service.update_single_item_status(item_id, status_update.status)
    if not updated:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, 
            detail=f"Roadmap item with ID {item_id} not found or update failed"
        )
    
    # If successful, fetch the updated item to return it
    # This is slightly inefficient (read-after-write) but simple for now
    items = roadmap_service.get_roadmap_items()
    updated_item = next((item for item in items if isinstance(item, dict) and item.get('id') == item_id), None)
    if updated_item is None:
         # Should not happen if update_single_item_status returned True, but handle defensively
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail="Failed to retrieve updated roadmap item after status change."
        )
